...
export class GetOffersUseCase
  implements UseCase<GetOfferRequestDto, Promise<Response>> {
  ...
  public async execute(request: GetOfferRequestDto): Promise<Response> {
    try {
      const paginatedOffers = await this.offerRepo.getOffers(request);
