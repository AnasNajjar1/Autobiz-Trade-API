...
import { Vehicle } from '../../../../../modules/catalog/domain/vehicle';
import equipmentsSchema from '../../../../../shared/schemas/equipments.json';
import { emailUsers } from './config';
import _ from 'lodash';
import { SendPartnerRequestProps } from '../SendPartnerRequestDto';
...
export class VpAutoMap implements Mapper<Vehicle> {
  public static toVpAuto({
    vehicle,
    uuid,
    saleComment,
    senderEmail,
  }: SendPartnerRequestProps): VpAutoDto {
    return {
      id: vehicle.id,
      ...
      gallery: vehicle.gallery()?.filter((o) => o.key && o.value),
      request: uuid,
      salesComment: saleComment,
/*       user: emailUsers(
        parseInt(vehicle.pointofsale?.autobizPosId),
