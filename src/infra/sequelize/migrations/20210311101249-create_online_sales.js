...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.query("CREATE OR REPLACE ALGORITHM = UNDEFINED VIEW onlineSales AS (SELECT `sale`.*, (SELECT   GROUP_CONCAT(CONCAT('{\n" +
        "  \"id\":\"', saleoffers.id, '\",\n" +
        ...
        "   \n" +
        "  }')) user " +
        ", `salebookmarks`.`id` AS `salebookmarks.id` FROM    " +
  ...
  down: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.query(`DROP VIEW  onlineSales`);
  },
};
