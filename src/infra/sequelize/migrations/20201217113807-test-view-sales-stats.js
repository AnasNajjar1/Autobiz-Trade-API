...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    CREATE PROCEDURE _test_salesStats_view()
    BEGIN
      START TRANSACTION;

      SET @fail = false;
      set @forSixteenDays = (SELECT SUBDATE(NOW(),16));
      ...
      #time passes : sales is closed for 16 days
      #it should have a ARCHIVED satus
      UPDATE sales SET endDateTime = @forSixteenDays WHERE id = @newSaleId;
      SET @status = (SELECT status FROM salesStats WHERE saleId = @newSaleId);
      IF @status <> "ARCHIVED" THEN
        SELECT 'FAIL : A VALIDATED sale ended 16 days ago should have a ARCHIVED satus';
        SET @fail = true;
      END IF;
