...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    BEGIN
      ...
      set @forFourteenHour = (SELECT DATE_SUB(NOW(),INTERVAL 14 HOUR));
      ...
      #time passes : sales is closed for 14 hours
      #it should have a FINISHEd satus
      UPDATE sales SET endDateTime = @forFourteenHour WHERE id = @newSaleId;
      SET @status = (SELECT status FROM salesStats WHERE saleId = @newSaleId);
      IF @status <> "FINISHED" THEN
        SELECT 'FAIL : A VALIDATED sale ended fourteen hour ago should have a FINISHED status';
        SET @fail = true;
      END IF;
